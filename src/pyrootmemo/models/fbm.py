...
class Fbm():
    ...
    def __init__(
        ...
        self.output = {"breakage_order": np.argsort(self.sort_order)}
    ...
    def calc_reduction_factor(
        ...
        Function creates a dictionary with the key `reduction_factor`. How 
        this is returned depends on the value of the `results` argument.
        ...
        factor = peak_force_output['peak_force'] / force_sum
        if isinstance(factor, Quantity):
            factor = factor.magnitude
        output = {'reduction_factor': factor}
        match Results(results).how:
            case ResultsType.ATTRIBUTE:
                self.output.update(output)
            case ResultsType.RETURN:
