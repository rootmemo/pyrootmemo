...
def create_weights(
    ...
    x : np.ndarray | Quantity
        array with observations
    weights : int | float | np.ndarray | None, optional
...
class PowerFit(BaseFit1D):
    ...
    def __init__(
        ...
        if lower is None:
            ...
            self.lower = create_quantity(lower, return_scalar = True)
        if upper is None:
            self.upper = np.max(x)
        else:
            self.upper = create_quantity(upper, return_scalar = True)
        ...
        if self.zero_variance is True:
            match self.model:
                ...
                case 'lognormal' | 'lognormal_corrected' | 'lognormal_uncorrected':
                    self.sdlog = 0.0
                case 'normal' | 'normal_strength' | 'normal_freesd' | 'normal_scaled':
            ...
            match self.model:
                ...
                case 'lognormal' | 'lognormal_corrected' | 'lognormal_uncorrected':
                    vals = np.array([np.array([
                        ...
                        f.sdlog])
                        for f in fits])
                case 'normal' | 'normal_strength' | 'normal_force' | 'normal_scaled':
                    vals = np.array([np.array([
                        nondimensionalise(f.multiplier, self.y0), 
                        f.exponent,
